...
import { BookOpen, Circle, CircleHelp, Cuboid, Cylinder, FolderOpen, Hammer, Link2, Maximize2, Menu, Pause, Play, RotateCw, Save, Shuffle, Trash2, Unlink2, X } from "lucide-react";
  ...
  return (
    <div className="pointer-events-none absolute inset-0 p-3 pt-[max(0.75rem,env(safe-area-inset-top))] pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:p-5">
      ...
      {open && <div className="pointer-events-auto absolute inset-x-3 bottom-[calc(4.75rem+env(safe-area-inset-bottom))] mx-auto max-h-[min(70dvh,34rem)] w-[calc(100%-1.5rem)] max-w-3xl overflow-y-auto rounded-toolbar border border-border bg-surface/95 p-3 shadow-toolbar backdrop-blur-md sm:inset-x-5 sm:bottom-[calc(5.25rem+env(safe-area-inset-bottom))] sm:w-[calc(100%-2.5rem)] sm:p-4">
        ...
        {activeTool === "scale" && <div className="mt-3 flex flex-wrap gap-2 border-t border-border pt-3"><Button variant="muted" disabled={!selected} onClick={() => transformSelected(Math.PI / 12, 1)}><RotateCw className="size-4" />Rotate</Button><Button variant="muted" disabled={!selected} onClick={() => transformSelected(0, 1.15)}><Maximize2 className="size-4" />Grow</Button><Button variant="muted" disabled={!selected} onClick={() => transformSelected(0, 0.87)}><Maximize2 className="size-4 rotate-180" />Shrink</Button>{!selected && <span className="self-center text-xs text-muted">Select a body in the scene first.</span>}</div>}
