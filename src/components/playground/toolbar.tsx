...
export function Toolbar() {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 p-3 pt-[max(0.75rem,env(safe-area-inset-top))] pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:p-5">
      ...
      <div className="pointer-events-auto absolute left-3 right-3 top-[calc(4.75rem+env(safe-area-inset-top))] z-10 mx-auto flex max-w-3xl items-center gap-2 rounded-toolbar border border-border/80 bg-surface/90 p-2 shadow-toolbar backdrop-blur-md sm:left-5 sm:right-5 sm:top-[calc(5rem+env(safe-area-inset-top))]"><Button variant="muted" className="flex-1 justify-center" disabled={!canUndo} onClick={undo}><Undo2 className="size-4" /> Undo</Button><Button variant="muted" className="flex-1 justify-center" disabled={!canRedo} onClick={redo}><Redo2 className="size-4" /> Redo</Button></div>
      ...
      {activeTool === "spawn" && <section className="pointer-events-auto absolute bottom-[calc(5.75rem+env(safe-area-inset-bottom))] left-3 right-3 z-10 mx-auto max-w-3xl rounded-toolbar border border-border/80 bg-surface/95 p-3 shadow-toolbar backdrop-blur-xl sm:bottom-[calc(6.25rem+env(safe-area-inset-bottom))] sm:left-5 sm:right-5 sm:p-4" aria-label="Spawn controls">
        <div className="flex flex-wrap items-center gap-2"><label className="flex items-center gap-2 text-xs font-semibold text-muted">Count<select value={spawnCount} onChange={(event) => setSpawnCount(Number(event.target.value))} className="h-10 rounded-sm border border-border bg-surface-2 px-2 text-fg">{[1, 2, 3, 5, 10].map((value) => <option key={value} value={value}>{value}</option>)}</select></label>{SHAPES.map(({ kind, label, icon: Icon }) => <Button key={kind} variant="muted" onClick={() => Array.from({ length: spawnCount }, () => spawn(kind))}><Icon className="size-4" /><span>{label}</span></Button>)}<Button variant="muted" onClick={scatter}><Shuffle className="size-4" /> Scatter</Button>{(["wall", "floor", "pillar", "room", "bridge"] as PresetKind[]).map((preset) => <Button key={preset} variant="ghost" onClick={() => spawnPreset(preset)}>{preset[0].toUpperCase() + preset.slice(1)}</Button>)}</div>
      </section>}

      <section className={cn("pointer-events-auto absolute bottom-[calc(5.75rem+env(safe-area-inset-bottom))] left-3 right-3 z-10 mx-auto max-h-[min(55dvh,32rem)] max-w-3xl overflow-y-auto rounded-toolbar border border-border/80 bg-surface/95 p-3 shadow-toolbar backdrop-blur-xl sm:bottom-[calc(6.25rem+env(safe-area-inset-bottom))] sm:left-5 sm:right-5 sm:p-4", activeTool === "select" && selected ? "translate-y-0 opacity-100" : "pointer-events-none translate-y-3 opacity-0")} aria-label="Selected object controls">
        <div className="mb-3 flex items-center justify-between border-b border-border pb-3"><div><p className="text-sm font-semibold">Selected object</p><p className="text-xs text-muted">Lock, duplicate, rename, or edit this piece.</p></div><Button variant="ghost" size="icon" onClick={() => setSelectedBodyId(null)} aria-label="Close object controls"><X className="size-4" /></Button></div>
        <div className="flex flex-wrap gap-2"><Button variant={selectedLocked ? "solid" : "muted"} onClick={toggleSelectedLock}>{selectedLocked ? <Unlock className="size-4" /> : <Lock className="size-4" />}{selectedLocked ? "Release lock" : "Lock in place"}</Button><Button variant="muted" onClick={() => { duplicateSelected(); setStatus("Copied exact object and spawned it nearby"); }}><Copy className="size-4" /> Duplicate</Button><Button variant="ghost" className="text-danger" onClick={() => { if (selectedBodyId) remove(selectedBodyId); setStatus("Object deleted"); }}><Trash2 className="size-4" /> Delete</Button><label className="flex items-center gap-2 text-xs font-semibold text-muted">Material<select value={selectedMaterial} onChange={(event) => setSelectedMaterial(event.target.value as MaterialKind)} className="h-10 rounded-sm border border-border bg-surface-2 px-3 text-xs font-semibold text-fg"><option value="wood">Wood</option><option value="steel">Steel</option><option value="glass">Glass</option></select></label></div>
        <div className="mt-3 grid gap-2 border-t border-border pt-3"><label className="text-xs font-semibold text-muted">Object name<input value={nameDraft} onChange={(event) => setNameDraft(event.target.value)} onBlur={() => selectedBodyId && renameBody(selectedBodyId, nameDraft)} className="mt-1 h-9 w-full rounded-sm border border-border bg-surface-2 px-2 text-xs text-fg" /></label>{selectedBody && <div className="grid grid-cols-3 gap-2">{(["position", "rotation", "scale"] as const).map((field) => <fieldset key={field} className="rounded-sm border border-border p-2"><legend className="px-1 text-[10px] font-semibold uppercase text-subtle">{field === "rotation" ? "Rotation °" : field}</legend>{[0, 1, 2].map((axis) => <input key={axis} type="number" step={field === "scale" ? "0.1" : "0.25"} value={field === "rotation" ? Number((selectedBody[field][axis] * 180 / Math.PI).toFixed(2)) : selectedBody[field][axis]} onChange={(event) => setSelectedTransform(field, axis as 0 | 1 | 2, Number(event.target.value))} className="mb-1 h-8 w-full rounded-sm border border-border bg-surface-2 px-1.5 text-xs text-fg" aria-label={`${field} ${axis === 0 ? "X" : axis === 1 ? "Y" : "Z"}`} />)}</fieldset>)}</div>}</div>
        <div className="mt-3 flex flex-wrap gap-2 border-t border-border pt-3"><span className="self-center text-xs font-semibold text-muted">Move</span>{(["x", "y", "z"] as const).map((axis) => <span key={axis} className="inline-flex gap-1 rounded-sm border border-border bg-surface-2 p-1"><Button variant="ghost" onClick={() => moveSelected(axis, -0.25)}>{axis.toUpperCase()}−</Button><Button variant="ghost" onClick={() => moveSelected(axis, 0.25)}>{axis.toUpperCase()}+</Button></span>)}<Button variant="muted" onClick={uprightSelected}><RotateCcw className="size-4" /> Upright</Button></div>
        <div className="mt-3 flex items-center justify-between border-t border-border pt-3"><label className="flex items-center gap-2 text-xs font-semibold text-muted"><input type="checkbox" checked={snapEnabled} onChange={(event) => setSnap(event.target.checked)} /> Snap <select value={snapStep} onChange={(event) => setSnap(snapEnabled, Number(event.target.value))} className="rounded-sm border border-border bg-surface-2 px-2 py-1 text-fg"><option value="0.1">0.1</option><option value="0.25">0.25</option><option value="0.5">0.5</option><option value="1">1</option></select></label><span className="text-[11px] text-muted">Drag X / Y / Z rings for free rotation.</span></div>
      </section>
