...
export function Toolbar() {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 p-3 pt-[max(0.75rem,env(safe-area-inset-top))] pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:p-5">
      ...
      {activeTool === "spawn" && <section className="pointer-events-auto absolute bottom-[calc(6.25rem+env(safe-area-inset-bottom))] left-1/2 z-10 w-[min(94vw,32rem)] -translate-x-1/2 rounded-toolbar border border-border/80 bg-surface/95 p-2 shadow-toolbar backdrop-blur-xl sm:bottom-[calc(6.75rem+env(safe-area-inset-bottom))] sm:p-3" aria-label="Spawn controls">
        <div className="grid grid-cols-3 gap-1.5"><label className="flex min-h-9 items-center justify-center gap-1 rounded-sm bg-surface-2 px-1 text-[11px] font-semibold text-muted">Count<select value={spawnCount} onChange={(event) => setSpawnCount(Number(event.target.value))} className="h-8 rounded-sm border border-border bg-surface px-1 text-fg">{[1, 2, 3, 5, 10].map((value) => <option key={value} value={value}>{value}</option>)}</select></label>{SHAPES.map(({ kind, label, icon: Icon }) => <Button key={kind} variant="muted" className="min-h-9 px-1 text-[11px]" onClick={() => Array.from({ length: spawnCount }, () => spawn(kind))}><Icon className="size-3.5" /><span>{label}</span></Button>)}<Button variant="muted" className="min-h-9 px-1 text-[11px]" onClick={scatter}><Shuffle className="size-3.5" /> Scatter</Button>{(["wall", "floor", "pillar", "room", "bridge"] as PresetKind[]).map((preset) => <Button key={preset} variant="ghost" className="min-h-9 px-1 text-[11px]" onClick={() => spawnPreset(preset)}>{preset[0].toUpperCase() + preset.slice(1)}</Button>)}</div>
      </section>}

      <section className={cn("pointer-events-auto absolute right-0 top-1/2 z-40 max-h-[min(72dvh,34rem)] w-[min(86vw,22rem)] -translate-y-1/2 overflow-y-auto rounded-l-toolbar border-y border-l border-border/80 bg-surface/95 p-3 shadow-toolbar backdrop-blur-xl transition-transform duration-300 ease-out sm:p-4", activeTool === "select" && selected ? "translate-x-0 opacity-100" : "pointer-events-none translate-x-[calc(100%+1rem)] opacity-0")} aria-label="Selected object controls">
