...
import { ArrowDown, ArrowLeft, ArrowRight, ArrowUp, BookOpen, Circle, CircleHelp, Cuboid, Cylinder, FolderOpen, Hammer, Link2, Lock, Maximize2, Menu, Pause, Play, RotateCw, Save, Shuffle, Trash2, Unlock, Unlink2, X } from "lucide-react";
import { useEffect, useState } from "react";
import { Button } from "@/components/ui/button";
import { Slider } from "@/components/ui/slider";
import { cn } from "@/lib/utils";
import { MATERIALS, usePlayground, type ConstructionTool, type MaterialKind, type PresetKind, type ShapeKind } from "./store";
...
export function Toolbar() {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 p-3 pt-[max(0.75rem,env(safe-area-inset-top))] pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:p-5">
      ...
      {open && <div className="pointer-events-auto absolute inset-x-3 bottom-[calc(4.75rem+env(safe-area-inset-bottom))] mx-auto max-h-[min(44dvh,25rem)] w-[calc(100%-1.5rem)] max-w-3xl overflow-y-auto rounded-toolbar border border-border/80 bg-surface/70 p-3 shadow-toolbar backdrop-blur-xl sm:inset-x-5 sm:bottom-[calc(5.25rem+env(safe-area-inset-bottom))] sm:max-h-[min(48dvh,30rem)] sm:w-[calc(100%-2.5rem)] sm:p-4">
        ...
        {activeTool === "spawn" && <div className="mt-3 flex flex-wrap items-center gap-2"><label className="flex items-center gap-2 text-xs font-semibold text-muted">Count <select value={spawnCount} onChange={(event) => setSpawnCount(Number(event.target.value))} className="h-11 rounded-sm border border-border bg-surface-2 px-2 text-fg"><option value="1">1</option><option value="2">2</option><option value="3">3</option><option value="5">5</option><option value="10">10</option></select></label>{SHAPES.map(({ kind, label, icon: Icon }) => <Button key={kind} variant="muted" onClick={() => Array.from({ length: spawnCount }, () => spawn(kind))} aria-label={`Drop ${spawnCount} ${label.toLowerCase()}${spawnCount > 1 ? "s" : ""}`}><Icon className="size-4" strokeWidth={1.75} /><span>{label} ×{spawnCount}</span></Button>)}<Button variant="muted" onClick={scatter}><Shuffle className="size-4" /><span>Scatter</span></Button>{(["wall", "floor", "pillar", "room", "bridge"] as PresetKind[]).map((preset) => <Button key={preset} variant="ghost" onClick={() => spawnPreset(preset)}>{preset[0].toUpperCase() + preset.slice(1)}</Button>)}</div>}
