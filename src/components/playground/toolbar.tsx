...
const SHAPES: { kind: ShapeKind; label: string; icon: typeof Circle }[] = [
  ...
  { kind: "trapezium", label: "Trapezium", icon: Cylinder },
  { kind: "plateau", label: "Plateau", icon: Cylinder },
  { kind: "arc", label: "Arc", icon: Circle },
  { kind: "semicircle", label: "Semi-circle", icon: Circle },
];
...
export function Toolbar() {
  ...
  const groupSelected = usePlayground((s) => s.groupSelected);
  const ungroupSelected = usePlayground((s) => s.ungroupSelected);
  ...
  return (
    <div className="pointer-events-none absolute inset-0 p-3 pt-[max(0.75rem,env(safe-area-inset-top))] pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:p-5">
      ...
      <section className={cn("pointer-events-auto absolute bottom-[calc(6.25rem+env(safe-area-inset-bottom))] left-1/2 z-40 h-[6.5rem] w-[min(94vw,42rem)] -translate-x-1/2 overflow-x-auto overflow-y-hidden rounded-toolbar border border-border/80 bg-surface/95 p-2 shadow-toolbar backdrop-blur-xl transition-all duration-300 ease-out sm:bottom-[calc(6.75rem+env(safe-area-inset-bottom))] sm:p-3", activeTool === "select" && selected ? "translate-y-0 opacity-100" : "pointer-events-none translate-y-4 opacity-0")} aria-label="Selected object controls">
        <div className="flex h-full min-w-max items-center gap-2"><div className="mr-1 shrink-0"><p className="text-sm font-semibold">Selected object</p><p className="text-[11px] text-muted">Drag the X / Y / Z handles.</p></div><Button variant={selectedLocked ? "solid" : "muted"} onClick={toggleSelectedLock}>{selectedLocked ? <Unlock className="size-4" /> : <Lock className="size-4" />}{selectedLocked ? "Release" : "Lock"}</Button><Button variant="muted" onClick={() => { duplicateSelected(); setStatus("Copied exact object and spawned it nearby"); }}><Copy className="size-4" /> Duplicate</Button><Button variant="muted" onClick={() => setStatus(groupSelected() ? "Welded structure grouped" : "Group requires welded objects")}><Group className="size-4" /> Group</Button><Button variant="muted" onClick={() => setStatus(ungroupSelected() ? "Structure ungrouped" : "Select a grouped structure first")}><Ungroup className="size-4" /> Ungroup</Button><Button variant="ghost" className="text-danger" onClick={() => { if (selectedBodyId) remove(selectedBodyId); setStatus("Object deleted"); }}><Trash2 className="size-4" /> Delete</Button><label className="flex items-center gap-2 text-xs font-semibold text-muted">Material<select value={selectedMaterial} onChange={(event) => setSelectedMaterial(event.target.value as MaterialKind)} className="h-9 rounded-sm border border-border bg-surface-2 px-2 text-xs font-semibold text-fg"><option value="wood">Wood</option><option value="steel">Steel</option><option value="glass">Glass</option></select></label><label className="flex items-center gap-2 text-xs font-semibold text-muted">Name<input value={nameDraft} onChange={(event) => setNameDraft(event.target.value)} onBlur={() => selectedBodyId && renameBody(selectedBodyId, nameDraft)} className="h-9 w-28 rounded-sm border border-border bg-surface-2 px-2 text-xs text-fg" /></label>{selectedBody && <div className="flex items-center gap-1 rounded-sm border border-border bg-surface-2 p-1">{(["x", "y", "z"] as const).map((axis) => <span key={axis} className="inline-flex gap-0.5"><Button variant="ghost" className="h-8 px-2" onClick={() => moveSelected(axis, -0.25)}>{axis.toUpperCase()}−</Button><Button variant="ghost" className="h-8 px-2" onClick={() => moveSelected(axis, 0.25)}>{axis.toUpperCase()}+</Button></span>)}</div>}<Button variant="muted" onClick={uprightSelected}><RotateCcw className="size-4" /> Upright</Button><Button variant="ghost" size="icon" onClick={() => setSelectedBodyId(null)} aria-label="Close object controls"><X className="size-4" /></Button></div>
      </section>

      <section className={cn("pointer-events-auto absolute bottom-[calc(6.25rem+env(safe-area-inset-bottom))] left-1/2 z-30 flex h-[8rem] w-[min(94vw,42rem)] -translate-x-1/2 items-center justify-center rounded-toolbar border border-border/80 bg-surface/95 p-3 shadow-toolbar backdrop-blur-xl transition-all duration-300 ease-out sm:bottom-[calc(6.75rem+env(safe-area-inset-bottom))]", activeTool === "scale" && selected ? "translate-y-0 opacity-100" : "pointer-events-none translate-y-4 opacity-0")} aria-label="Scale controls"><div className="flex w-full items-center gap-3"><div className="shrink-0"><p className="text-sm font-semibold">Shape object</p><p className="text-[11px] text-muted">Thin, fat, flat, slim or thick</p></div><Button variant="muted" size="icon" className="size-11 shrink-0" onClick={() => usePlayground.getState().transformSelected(0, 0.9)} aria-label="Decrease scale"><Minus className="size-5" /></Button>{selectedBody && <div className="grid min-w-0 flex-1 gap-1"><label className="flex items-center gap-2 text-[10px] font-semibold uppercase text-muted">X <input type="range" min="0.25" max="4" step="0.05" value={selectedBody.scale[0]} onChange={(event) => setSelectedTransform("scale", 0, Number(event.target.value))} className="min-w-0 flex-1 accent-accent" /></label><label className="flex items-center gap-2 text-[10px] font-semibold uppercase text-muted">Y <input type="range" min="0.25" max="4" step="0.05" value={selectedBody.scale[1]} onChange={(event) => setSelectedTransform("scale", 1, Number(event.target.value))} className="min-w-0 flex-1 accent-accent" /></label><label className="flex items-center gap-2 text-[10px] font-semibold uppercase text-muted">Z <input type="range" min="0.25" max="4" step="0.05" value={selectedBody.scale[2]} onChange={(event) => setSelectedTransform("scale", 2, Number(event.target.value))} className="min-w-0 flex-1 accent-accent" /></label></div>}<span className="min-w-12 text-center text-xs font-semibold text-muted">{selectedBody ? `${selectedBody.scale[0].toFixed(1)}×` : "—"}</span><Button variant="solid" size="icon" className="size-11 shrink-0" onClick={() => usePlayground.getState().transformSelected(0, 1.1)} aria-label="Increase scale"><Plus className="size-5" /></Button></div></section>
